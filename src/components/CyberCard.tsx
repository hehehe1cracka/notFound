...
const CyberCard: React.FC<CyberCardProps> = ({ image, title, subtitle, onClick }) => {
    return (
        <StyledWrapper onClick={onClick}>
            <div className="container noselect">
                <div className="canvas">
                    ...
                    {/* Added more trackers if needed, simplified for speed */}

                    <div id="card" style={{ backgroundImage: image ? `url(${image})` : 'linear-gradient(45deg, #1a1a1a, #262626)', backgroundSize: 'cover', backgroundPosition: 'center' }}>
                        {/* Dark overlay for text readability if image exists */}
                        {image && <div className="absolute inset-0 bg-black/60 z-0"></div>}
                        ...
                        <div className="card-content z-10">
                            ...
                            <p id="prompt">VIEW STARTUP</p>
                            <div className="title" dangerouslySetInnerHTML={{ __html: title.replace(' ', '<br/>') }}></div>
...
const StyledWrapper = styled.div`
  .container {
    position: relative;
    width: 300px; /* Increased from 190px for better visibility */
    height: 400px; /* Increased from 254px */
    transition: 200ms;
    cursor: pointer;
  }
  ...
  .tr-4 { grid-area: 2 / 1 / 3 / 2; }
  .tr-5 { grid-area: 2 / 2 / 3 / 3; }
  .tr-6 { grid-area: 2 / 3 / 3 / 4; }
  .tr-7 { grid-area: 3 / 1 / 4 / 2; }
  .tr-8 { grid-area: 3 / 2 / 4 / 3; }
  .tr-9 { grid-area: 3 / 3 / 4 / 4; }

  .tr-1:hover ~ #card { transform: rotateX(20deg) rotateY(-10deg); }
  .tr-2:hover ~ #card { transform: rotateX(20deg) rotateY(0deg); }
  .tr-3:hover ~ #card { transform: rotateX(20deg) rotateY(10deg); }
  ...
  .tr-7:hover ~ #card { transform: rotateX(-20deg) rotateY(-10deg); }
  .tr-8:hover ~ #card { transform: rotateX(-20deg) rotateY(0deg); }
  .tr-9:hover ~ #card { transform: rotateX(-20deg) rotateY(10deg); }
